import { PrismaClient, RoleEnum, EmploymentStatus, ContractType, AttendanceMethod, AttendanceStatus } from '@prisma/client';
...
async function main() {
  ...
  for (const roleName of [RoleEnum.SUPER_ADMIN, RoleEnum.HR_ADMIN, RoleEnum.MANAGER, RoleEnum.EMPLOYEE]) {
  ...
  for (const perm of permissionsData) {
    ...
    await prisma.rolePermission.create({
      data: {
        roleId: rolesMap.get(RoleEnum.SUPER_ADMIN).id,
  ...
  await prisma.user.create({
    data: {
      email: 'hr@hrms.com',
      passwordHash,
      roleId: rolesMap.get(RoleEnum.HR_ADMIN).id,
      employeeId: empHR.id,
      isActive: true,
    },
  });
  ...
  await prisma.user.create({
    data: {
      email: 'manager@hrms.com',
      passwordHash,
      roleId: rolesMap.get(RoleEnum.MANAGER).id,
      employeeId: empManager.id,
      isActive: true,
    },
  });

  // Standard Employee (Software Engineer)
  ...
  await prisma.user.create({
    data: {
      email: 'employee@hrms.com',
      passwordHash,
      roleId: rolesMap.get(RoleEnum.EMPLOYEE).id,
      employeeId: empEmployee.id,
      isActive: true,
    },
  });

  // Set Manager for department Tech
  ...
  // 10. Seed Trusted Attendance CIDR Networks
  ...
  // 12. Seed Sample Attendance Records for today & past days
  ...
  // 13. Seed Tax & Insurance Configurations
